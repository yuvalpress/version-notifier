...
func Notify(user, repo, url, oldVer, newVer, updateLevel string) {
	...
	blockType, notes := release_notes.GetReleaseNotes(url)
	if notes != "" {
		_, _, err := slackClient.PostMessage(
			...
			slack.MsgOptionBlocks(slack.NewSectionBlock(slack.NewTextBlockObject(blockType, notes, false, false), nil, nil)),
		)
		if err != nil {
